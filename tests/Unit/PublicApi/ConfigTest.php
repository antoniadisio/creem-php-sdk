...
test('config applies overrides and normalizes inputs', function (): void {
    ...
    expect($config->apiKey())->toBe('creem_test_123')
...
test('config normalizes blank user agent suffixes to null', function (): void {
    $config = new Config('creem_123', userAgentSuffix: '   ');
...
test('config strips control characters from the user agent suffix', function (): void {
    $config = new Config('creem_123', userAgentSuffix: " app\r\nv1\t ");
...
test('config resolves the default environment base url when no override is provided', function (): void {
    $config = new Config('creem_test_123', Environment::Test);
...
test('config uses redacted values for debug output string casts and serialization', function (): void {
    $config = new Config('creem_secret_1234', userAgentSuffix: 'integration-suite');
    $debugInfo = $config->__debugInfo();
    $serialized = serialize($config);

    expect($debugInfo['apiKey'] ?? null)->toBe('creem_****1234')
        ->and((string) $config)->toContain('creem_****1234')
        ->and((string) $config)->not->toContain('creem_secret_1234')
...
test('config rejects unserialization to avoid restoring redacted credentials', function (): void {
    $config = new Config('creem_secret_1234');
...
test('config accepts creem test api keys only for the test environment', function (): void {
    $config = new Config('creem_test_123', Environment::Test);
...
function invalidConfigValues(): array
{
    return [
        ...
        'zero timeout' => [
            static fn(): Config => new Config('creem_123', timeout: 0),
            'The Creem request timeout must be greater than zero.',
        ],
        'malformed api key' => [
            static fn(): Config => new Config('not-a-creem-key'),
            'The Creem API key must start with "creem_test_" or "creem_".',
        ],
        'creem test key on production environment' => [
            static fn(): Config => new Config('creem_test_123', Environment::Production),
            'Creem test API keys starting with "creem_test_" require Environment::Test.',
        ],
        'creem production key on test environment' => [
            static fn(): Config => new Config('creem_123', Environment::Test),
            'Creem production API keys starting with "creem_" require Environment::Production.',
        ],
        'negative timeout' => [
            static fn(): Config => new Config('creem_123', timeout: -1),
            'The Creem request timeout must be greater than zero.',
        ],
        'blank base url' => [
            static fn(): Config => new Config('creem_123', baseUrl: '   '),
            'The Creem base URL override cannot be blank.',
        ],
        'non https base url' => [
            static fn(): Config => new Config('creem_123', baseUrl: 'http://example.test'),
            'The Creem base URL override must be a valid HTTPS URL.',
        ],
        'untrusted base url without opt in' => [
            static fn(): Config => new Config('creem_123', baseUrl: 'https://example.test'),
            'The Creem base URL override host is not trusted. Pass allowUnsafeBaseUrlOverride: true to allow non-Creem hosts.',
        ],
        'malformed base url' => [
            static fn(): Config => new Config('creem_123', baseUrl: 'not a url'),
            'The Creem base URL override must be a valid HTTPS URL.',
        ],
    ];
}
