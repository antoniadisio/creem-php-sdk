...
$request = static function (array $values): ValidateLicenseRequest {
    return new ValidateLicenseRequest(
...
return [
    ...
    'sdk_call' => '$client->licenses()->validate(new ValidateLicenseRequest($licenseKey, $instanceId), $idempotencyKey)',
    'http_method' => 'POST',
    ...
    'inputs' => [
        ...
        Playground::field('licenses.validate.idempotencyKey', 'Idempotency key', 'string'),
    ],
    'idempotency_key_path' => 'licenses.validate.idempotencyKey',
    'persist_outputs' => [
    ...
    'build_inputs' => static fn (array $values): array => [
        ...
        'idempotencyKey' => Playground::value($values, 'licenses.validate.idempotencyKey'),
    ],
    'build_request_payload' => static fn (array $values): array => $request($values)->toArray(),
    'run' => static fn (Client $client, array $values) => $client->licenses()->validate(
        $request($values),
        Playground::stringValue(
            Playground::value($values, 'licenses.validate.idempotencyKey'),
            'licenses.validate.idempotencyKey',
        ),
    ),
];
