...
- Do not commit local-only planning files or machine-specific files such as `.env`, `.spec/`, `.codex/`, personal local workflow files, `vendor/`, `node_modules/`, or IDE settings. Playground runtime artifacts such as `playground/state.local.json`, `playground/captures/`, and `playground/*.local.json` stay local via `playground/.gitignore`.
