...
The SDK is a PHP 8.4 library with PSR-4 autoloading under `src/` (`Creem\\`). Keep the public API stable through `src/Client.php`, `src/Config.php`, resource wrappers in `src/Resource/`, shared API enums in `src/Enum/`, and typed DTOs in `src/Dto/`. The committed Rector config intentionally skips automatic type-declaration inference on `src/Client.php`, `src/Config.php`, and `src/Resource/*`, so public signature changes there stay under manual review. Transport and hydration details belong in `src/Internal/`; typed failures live in `src/Exception/`. Tests are in `tests/Unit/`, with reusable JSON fixtures in `tests/Fixtures/Responses/`, a Pest bootstrap in `tests/Pest.php`, and shared helper support in `tests/TestCase.php` plus `tests/Support/`. `tests/Integration/` exists for future end-to-end coverage. Contract references live in `spec/creem-openapi.json` and `docs/openapi-audit.md`. Human contributor guidance belongs in `CONTRIBUTING.md`.
...
- `composer rector` applies the committed Rector rules for PHP 8.4-targeted refactors and safe code-quality cleanup. Type-declaration rules apply to internal code and tests, while the public facade/config/resource API is path-skipped for manual review.
- `composer rector:check` verifies that Rector would make no further changes.
- `composer test` runs the Pest unit suite from `tests/Unit/`.
...
- `composer qa` runs the local fix-first QA flow in this order: Rector, Pint fixes, PHPStan, then Pest.
- `composer qa:check` runs the non-mutating verification flow in this order: Rector dry-run, Pint check, PHPStan, then Pest.

`composer install` and `composer update` resolve against the committed Composer platform pin (`php: 8.4.0`), so dependency changes must stay compatible with the PHP 8.4 support floor and the CI lockfile target.
